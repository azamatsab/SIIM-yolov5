...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        return torch.from_numpy(img), labels_out, self.img_files[index], shapes, img_labels
    ...
    def collate_fn4(batch):
        img, label, path, shapes, img_labels = zip(*batch)  # transposed
        n = len(shapes) // 4
        img4, label4, im_label4, path4, shapes4 = [], [], [], path[:n], shapes[:n]
        ...
        for i in range(n):  # zidane torch.zeros(16,3,720,1280)  # BCHW
            ...
            if random.random() < 0.5:
                ...
                im_l = img_labels[i] + img_labels[i + 1] + img_labels[i + 2] + img_labels[i + 3]
                im_l[im_l > 0] = 1
            img4.append(im)
            label4.append(l)
            im_label4.append(im_l)
